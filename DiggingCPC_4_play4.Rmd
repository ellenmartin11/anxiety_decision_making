...
one_long_file_z <- paste0(data_path,"z\\THP_uk_DiggingHappy4_z_n499.csv") #zscored

# parameters and model fits data
happy_file_one_z <- paste0(data_path, "modelBased\\z\\happy_params4_play4_z_n499.csv")
# 6 parameters
happy_file_one_z_6 <- paste0(data_path, "modelBased\\z\\happy_params6_play4_z_n499.csv")
...
happyData1 <- happyData1 %>%
  mutate(GAD_bin = cut(gad7_total, breaks = c(-Inf, 4, 9, 14, 21), labels = c("0-4", "5-9", "10-14", "15-21")),
      ...
      GAD_binary = case_when(gad7_total >= 5 ~ "GAD 5+",
                             gad7_total < 5 ~ "GAD <5"),
      PHQ_binary = case_when(phq8_total >= 6 ~ "PHQ 6+",
                             phq8_total < 6 ~ "PHQ <6"))
...
Read this in and then return to main script. 
```{r}
...
summary_data <- risky_data %>%
  ...
  ggplot(summary_data, aes(x = Trial, y = mean_percent_risky, color = Trial)) +
    ...
    geom_errorbar(aes(ymin = mean_percent_risky - se_percent_risky, ymax = mean_percent_risky + se_percent_risky),
                  width = 0.2) +
    labs(
      title = paste0("% Risky Choices (Play 4) N=",sample_size,""),
      x = "Trial Type",
      y = "% Risky Choices"
    ) +
    theme_minimal() +
...
ggplot(summary_bins, aes(x = GAD_bin, y = mean_sd_Happy_filled)) +
    ...
    labs(
      title = "Happiness by Anxiety (Play 4) N=458",
      x = "Anxiety (GAD score)",
...
summary_params <- happyData1 %>%
  ...
  ggplot(summary_params, aes(x = parameter, y = mean_value, fill = GAD_binary)) +
    ...
    theme_minimal() +
    poster_theme + 
    scale_fill_manual(values = c("GAD 5+" = "#00BD9D", "GAD <5" = "#8BD7D2")) +
  ...
  ggplot(summary_params, aes(x = parameter, y = mean_value, fill = PHQ_binary)) +
    ...
    theme_minimal() +
    poster_theme + 
    scale_fill_manual(values = c("PHQ 6+" = "red", "PHQ <6" = "pink")) +
    scale_x_discrete(labels = c("ev_chosen_multi_z" = "EV", "rpe_chosen_multi_z" = "RPE"))
```
## correlations between parameters and symptoms

```{r}
cor.test(summary_zHappy_1$GAD_score,summary_zHappy_1$rpe,method="spearman") # no correlation between GAD and rpe
cor.test(summary_zHappy_1$GAD_score,summary_zHappy_1$ev,method="spearman") #no correlation between GAD and ev
cor.test(summary_zHappy_1$PHQ_score,summary_zHappy_1$rpe,method="spearman") # no correlation between PHQ and RPE
cor.test(summary_zHappy_1$PHQ_score,summary_zHappy_1$ev,method="spearman") # no correlation between PHQ and EV
```
...
combined_risky <- na.omit(combined_risky)
  ...
  ggplot(combined_risky, aes(x = Future, y = mean_risky,  color = GAD_binary, group = GAD_binary)) +
    ...
    scale_color_manual(values = c("GAD 5+" = "#00BD9D", "GAD <5" = "#8BD7D2")) +
    labs(
...
combined_risky <- bind_rows(positive_risky, negative_risky)
  ...
  ggplot(combined_risky, aes(x = Future, y = mean_risky,  color = PHQ_binary, group = PHQ_binary)) +
    ...
    scale_color_manual(values = c("PHQ 6+" = "red", "PHQ <6" = "pink")) +
    labs(
      title = "% Risky Choices based on Future (Play 4) N=1922",
      x = "Future",
      y = "% Risky Choices"
...
# Happiness Based on Future and Current - Split by GAD 
```{r}
current_pos_future_pos <- happyData1 %>%
  filter(NextIsland == 2 & LastIsland == 2 & Trial == "Gain") %>%
  group_by(userKey, GAD_binary) %>%
...
#Happiness based on gamble decisoins
...
### Happiness Based on Choice x Domain
...
ggplot(summary_choice, aes(x = Choice, y = mean_happy, color = Trial)) +
  ...
  geom_errorbar(aes(ymin = mean_happy - sem_happy, ymax = mean_happy + sem_happy),
                position = position_dodge(1), width = 0.2) + 
  geom_point(aes(y = mean_predhappy), color = "lightblue", shape = 8, size = 3, position = position_dodge(1)) +
  geom_hline(yintercept = 0, linetype = "dotted", color = "black") +  # Add a dotted line at y=0
  labs(
...
ggplot(overall_mean_risky_ev_data_gad, aes(x = abs_RiskyEV, y = mean_percent_risky_all, color = GAD_binary, linetype = Trial)) +
  ...
  theme_minimal() +
  ylim(0, 100) +
  scale_color_manual(values = c("GAD 5+" = "#00BD9D", "GAD <5" = "#8BD7D2")) +  # Custom colors for GAD groups
...
ggplot(overall_mean_risky_ev_data_phq, aes(x = abs_RiskyEV, y = mean_percent_risky_all, color = PHQ_binary, linetype = Trial)) +
  ...
  theme_minimal() +
  ylim(0, 100) +
  scale_color_manual(values = c("PHQ 6+" = "red", "PHQ <6" = "pink")) +  # Custom colors for GAD groups
  scale_linetype_manual(values = c("Gain" = "solid", "Loss" = "dotted")) + 
  poster_theme 
...
length(unique(happyData1_demo$userKey)) #311
```
...
summary_data <- risky_data %>%
  ...
  ggplot(summary_data, aes(x = Trial, y = mean_percent_risky, color = gender_group)) +
    ...
    geom_errorbar(aes(ymin = mean_percent_risky - se_percent_risky, ymax = mean_percent_risky + se_percent_risky),position=position_dodge(0.8),
                  width = 0.2) +
    labs(
      title = "% Risky Choices (Play 1) N=311",
      x = "Trial Type",
      y = "% Risky Choices"
    ) +
    theme_minimal() +
